...
let allCaps = [];
runners.map(upName => allCaps.push (upName.first_name.toUpperCase()))
...
let largeShirts = [];
largeShirts.push(runners.filter(size => (size.shirt_size === "L")));
console.log(largeShirts);

// ==== Challenge 4: Use .reduce() ====
// The donations need to be tallied up and reported for tax purposes. Add up all the donations into a ticketPriceTotal array and log the result
let ticketPriceTotal = [];
ticketPriceTotal.push(runners.reduce((total, amount) => {
    total = total + amount.donation
    return total
},0));
